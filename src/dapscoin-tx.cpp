...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBP)
        {
            ...
            //decode bulletproofs input
            string strHexTx(argv[1]);
            if (IsHex(strHexTx)) {
                CRangeProofInput rpi;
                ...
                if (rpi.blinds.size()/32 <= MAX_VOUT && rpi.blinds.size()/32 == rpi.amounts.size()) {
                    memset(blinds, 0, rpi.blinds.size());
                    for (i = 0; i < rpi.blinds.size()/32; i++) {
                        memcpy(&blinds[i][0], &rpi.blinds[0] + i * 32, 32);
                        blind_ptr[i] = blinds[i];
                        values[i] = rpi.amounts[i];
                    }
